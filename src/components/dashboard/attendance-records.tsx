                  ...
                  <TableBody>
                    {paged.map(a => (
                      <TableRow key={a.id} className="cursor-pointer hover:bg-emerald-50"
                        ...
                        <TableCell className="text-xs py-2 whitespace-nowrap">{a.student?.name || '-'}</TableCell>
                        <TableCell className="text-xs py-2 whitespace-nowrap">{a.student?.class?.name || '-'}</TableCell>
                        <TableCell className="text-xs py-2 whitespace-nowrap hidden md:table-cell">{a.date ? formatDateShort(a.date) : '-'}</TableCell>
                        <TableCell className="text-xs py-2 whitespace-nowrap">{a.checkInTime ? formatTimeWIB(a.checkInTime) : '-'}</TableCell>
                        <TableCell className="text-xs py-2 whitespace-nowrap hidden sm:table-cell">{a.checkOutTime ? formatTimeWIB(a.checkOutTime) : '-'}</TableCell>
                        <TableCell className="py-2 whitespace-nowrap"><Badge className={`${getStatusColor(a.status as any)} text-xs`}>{a.status}</Badge></TableCell>
                        <TableCell className="py-2 whitespace-nowrap hidden lg:table-cell">{a.geoVerified ? <CheckCircle className="h-4 w-4 text-green-500" /> : <XCircle className="h-4 w-4 text-gray-300" />}</TableCell>
                      </TableRow>
                    ))}
